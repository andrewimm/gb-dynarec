pub enum Op {
  ...
  AddSP(i8),
  LoadAToMemory(u16, bool),
  LoadAFromMemory(u16, bool),
  LoadToHighMem,
  LoadFromHighMem,
  LoadStackOffset(i8),
  LoadToStackPointer,  
...
impl std::fmt::Display for Op {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      Op::LoadAToMemory(addr, _) => f.write_fmt(format_args!("LD ({:#06X}), A", addr)),
      Op::LoadAFromMemory(addr, _) => f.write_fmt(format_args!("LD A, ({:#06X})", addr)),
